...
or `Nullable(Int64)` schema and an appropriate row cap. The selected physical
storage follows that schema; nullable imports preserve every decoded NULL
position.
...
cached metrics on typed file corruption, schema-nullability mismatch, caller
row-cap, or configured table-limit failures. Success registers only the fully
validated `Int64` or `Nullable(Int64)` table, retaining its exact NULL positions
and caller-supplied row cap.
