...
4. A query engine plans scans, filters, projections, grouping, aggregation, sorting, and limits. A per-batch execution context owns predicate, planning, grouped-value, result, and operator accounting; grouping and sorting use deterministic temporary index runs when their in-memory working set fills, with fixed-capacity run metadata and incremental balanced merges.
