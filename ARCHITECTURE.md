...
owns the corresponding non-nullable `Int64` and `Float64` `MAX` reduction boundaries, including
first-occurrence Float64 ties and signed zero. The engine retains SQL
...
Bool key, plus sole non-nullable `Int64` or `Float64` `MAX` grouped by that key, over large filtered
row sets with scoped workers admitted by one process-wide nonblocking budget.
