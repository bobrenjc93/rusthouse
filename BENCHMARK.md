...
All child processes use one bounded runner. Validation children have a 30-second deadline and 64 KiB caps on each output stream. Correctness children have a 60-second deadline, an 8 MiB stdout cap, and a 1 MiB stderr cap. Timing children have a 120-second deadline, a 16 MiB stdout cap, and a 1 MiB stderr cap. Stdin delivery and output draining run concurrently so a child cannot block the harness on a full pipe. Supervision continues until both the direct child and all I/O workers finish, so a background descendant cannot escape the deadline or output caps by outliving its parent. Before accepting any completed child, the runner also terminates the remaining process group; cleanup errors reject the run. A deadline or cap breach terminates a still-running process group, reports the phase and limit (including deterministic truncation length for output floods), rejects the run with score zero, and exits nonzero. The same values are recorded under `execution_limits` in details JSON.
