...
The raw schema-version 3 artifact at `benchmark/results/default-20260729.json` is the retained record behind the repository's benchmark claims. It reports a default run with seed `20260729`, release binaries, the pinned ClickHouse identity, and 24 passed correctness gates. Its raw timings arithmetically produce a sustained score of 99.705102 and a startup-inclusive score of 100.000000; 23 of 24 sustained cases and all 24 startup-inclusive cases reach the parity cap.
...
Check the artifact's structural and arithmetic consistency without a ClickHouse or RustHouse executable:

~~~bash
cargo run --release --features benchmark-verifier \
  --bin clickhouse-parity-bench -- \
  --verify-details benchmark/results/default-20260729.json
~~~

The checker requires the exact workload/row-count matrix for the declared mode and rejects duplicate, missing, renamed, or re-family cases. It checks fixed mode-configuration claims and the reported ClickHouse identity, derives per-query samples from raw batch samples, reapplies sample-count and stability gates, and recomputes every median, ratio, saturation count, and hierarchical score. Hierarchical scores allow an absolute `1e-9` tolerance because Rust's platform `ln`/`exp` implementations are not bit-identical; other derived fields remain exact. Unknown, missing, or mismatched reported fields fail closed.

This is an arithmetic consistency check, not provenance or correctness authentication. A self-consistent artifact can be fabricated. The command does not prove the claimed seed, executable paths or identities, generated SQL/data, correctness outputs, or that either engine was executed. Repository history and review remain the trust anchor for the checked-in artifact; independent benchmark reproduction remains necessary for decision-grade evidence. No engine path is read or executed during the check.
...
Tests cover generator reproducibility, runtime-seed variation, dataset-shape and workload-diversity invariants, CSV normalization, separate correctness gating, equal engine amplification, positive amortized timings, unstable-sample rejection, score saturation detection, family/scale weighting, offline consistency replay, exact case-matrix enforcement, score portability tolerance, unauthenticated metadata messaging, and sample, score, configuration, and identity tampering.
