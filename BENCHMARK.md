...
`build.rs` embeds the source commit and dirty state, rustc version, compilation target, and Cargo profile in both binaries. A live checkout obtains source provenance from Git and watches the complete package tree, including newly added untracked files. A Cargo source package uses its generated `.cargo_vcs_info.json`; a Git-free archive without that file must supply both `RUSTHOUSE_BUILD_SOURCE_COMMIT` and `RUSTHOUSE_BUILD_SOURCE_DIRTY=true|false`. Missing or partial provenance fails the build.

Before validation or timing, the harness copies both configured executables into a private staging directory, syncs them, removes write access, and uses only those copies for every child process. It hashes the staged RustHouse executable, requests its private build attestation, and requires every embedded value to match the benchmark binary. It also records the runtime host platform and `uname` description. A dirty build remains explicitly marked rather than being represented as its clean commit. After all timing processes finish, both staged executable hashes, both version/attestation responses, and the host identity are checked again. Replacing a configured path after staging cannot change the executable under test. Missing, malformed, changed, or inconsistent provenance rejects the entire run without retaining a new report.

Details schema version 3 retains a canonical `suite_manifest` object and its SHA-256. Canonical bytes are the compact UTF-8 JSON bytes emitted in the documented field order, with no trailing newline: manifest version, mode, seed, timing settings, row counts, then cases in execution order. Every case contains its workload, family, row count, derived dataset seed, setup SQL SHA-256, and query SQL SHA-256. The same two SQL digests are repeated beside that case's timing samples so an auditor can bind generated inputs and queries to the manifest.
