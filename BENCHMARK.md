...
Progress is written to stderr. Stdout is exactly one compact Burner JSON object with score, summary, evidence, and suggestions. Its score is the primary sustained-work score; summary and evidence also name the end-to-end score. The --details option writes schema-versioned JSON containing the timing method and limitations, amplification, correctness count, raw batch and per-query samples, medians, both ratios and scores, paths, seed, mode, and complete provenance. The report is written to a new file in the destination directory, flushed and synced, atomically renamed over the destination, and followed by a directory sync. The previous report is retained until that sync succeeds and is restored if it fails; a rejected first report is removed. An incomplete or rejected report is never installed at the requested path.
...
Before validation or timing, the harness records its own executable SHA-256, copies both configured executables into a private staging directory, syncs them, removes write access, and uses only those copies for every child process. It hashes the staged RustHouse executable, requests its private build attestation, and requires every embedded value, including the build-configuration fingerprint, to match the benchmark binary. It also records the runtime host platform and `uname` description. After all timing processes finish, the harness and both staged executable hashes, both version/attestation responses, and the host identity are checked again. Replacing a configured path after staging cannot change the executable under test. Missing, malformed, changed, or inconsistent provenance rejects the entire run without retaining a new report.
