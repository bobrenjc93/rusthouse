...
Progress is written to stderr. Stdout is exactly one compact Burner JSON object with score, summary, evidence, and suggestions. Its score is the primary sustained-work score; summary and evidence also name the end-to-end score. The --details option writes schema-versioned JSON containing the timing method and limitations, amplification, correctness and amplified-preflight counts, each preflight's captured byte counts and limit, raw batch and per-query samples, medians, both ratios and scores, paths, seed, mode, and ClickHouse identity. Setup, execution, version, checksum, capture-limit, parse, correctness, timing-stability, or full default-suite saturation failures still emit the one object with score zero and exit nonzero.
...
## Correctness gate, amplified preflight, and normalization

Correctness, amplified preflight, and timing use separate processes. Before any timing for a case, the harness runs setup plus one unamplified query on each engine and captures both outputs. It then runs the exact setup-plus-256-query batch used by primary timing on both engines with stdout captured outside the timer. Each engine's preflight stdout is limited to 16 MiB; crossing the limit terminates the process and rejects the run.

The preflight parser treats the captured stream as repeated CSV result documents, using the unamplified result's row count to preserve unambiguous document boundaries. It requires exactly 256 documents from each engine. Every document must match that engine's unamplified expected result and the same repetition from the peer engine under the normal typed comparison rules. The timing gate opens only after both the unamplified comparison and amplified preflight pass. Amplified and end-to-end sample acceptance require that open gate. Any process, count, capture, parse, or comparison failure rejects the entire run; failed or absent gates cannot contribute timings.
...
Anti-gaming properties are the fixed external ClickHouse identity, configurable runtime seeds, multiple scales, deliberately conflicting data shapes, selective and nonselective predicates, two grouping cardinalities, deterministic query ordering, alternating engine order, separate fail-closed correctness gates, exact amplified-output preflights, identical per-engine amplification, retained raw samples, conservative per-case caps, and equal family/scale weighting. No single special-case query, favorable seed, or duplicated workload can legitimately stand in for the suite.
