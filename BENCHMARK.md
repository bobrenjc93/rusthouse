...
`build.rs` embeds the source commit and dirty state, rustc version, compilation target, and Cargo profile in both binaries. A live checkout obtains source provenance from Git and watches every source-bearing top-level path while excluding `.git`, `.burner`, and `target` output. It also watches the worktree HEAD/index and the resolved symbolic branch ref; packed refs use the existing packed-ref file and containing refs directory. This makes commits without source-file changes refresh the attestation without missing-path rebuild loops. A Cargo source package prefers the commit and optional dirty flag in its generated `.cargo_vcs_info.json`, even when extracted below an unrelated repository; packages produced with `--allow-dirty` remain dirty. Git provenance is accepted only when the package directory is that repository's root. A Git-free archive without packaged metadata must supply both `RUSTHOUSE_BUILD_SOURCE_COMMIT` and `RUSTHOUSE_BUILD_SOURCE_DIRTY=true|false`. Missing or partial provenance fails the build.
