...
Tests cover per-profile reproducibility, runtime-seed variation, physical schema shapes, execution of every profile query, projection metadata, CSV normalization, separate correctness gating, equal engine amplification, the calibrated 256 repetitions in every matrix cell, positive amortized timings, unstable-sample rejection, score saturation detection, equal profile/seed/family/scale weighting, and fail-closed matrix validation.

## Timing and calibration

The primary sample starts one process, creates and inserts one profile dataset, and executes the identical workload query 256 times against that in-memory table. The calibrated 256 repetitions apply independently to every profile/seed/workload/scale cell and are never divided as the matrix expands. This keeps repeated analytical execution dominant over the fresh process and CREATE/INSERT cost instead of turning the primary score into a second startup-inclusive measurement.

For each case, both engines receive the exact same setup SQL, query SQL, and 256 repetitions; the runner rejects a mismatch. Stdout goes to the null sink for timing processes. Total positive wall time is divided by 256, so startup and setup remain present at 1/256. Warmup process pairs are discarded, engine order alternates, and the median of measured samples is used.

The fixed 256-query per-cell amplification is the calibration for both quick and default modes. The total suite work is deterministic and bounded by the explicit matrix, sample counts, and this constant; there is no engine-dependent adaptive stopping. The harness deliberately performs no startup subtraction: subtracting independently noisy process measurements can create zero, negative, or highly unstable derived timings. Samples must remain positive, and a greater-than-10x max/min spread rejects the run.
...
Amplification measures repeated work on one loaded in-memory table. It can benefit CPU caches and repeated planning paths, does not model concurrency, and retains 1/256 of process startup and setup. The separately reported startup-inclusive score must be consulted for one-shot CLI use. Neither metric isolates only an execution kernel. Adding profiles and seeds intentionally increases total suite work to preserve the established sustained calibration for every scored cell.
