...
`QueryLimits` bounds hash-join build rows, join build and expanded-output
state, rows in each window partition, cumulative window maps/indexes/outputs
and accumulator state, and materialized output rows. Limits can be set as
database defaults or changed on a session. The HTTP service additionally
