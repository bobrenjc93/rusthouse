...
- GROUP BY and HAVING over group keys, SELECT aliases, aggregates, and literals
...
Identifiers are unquoted and case-insensitive; TRUE and FALSE are reserved Boolean literals and cannot be column names or SELECT aliases. String literals use single quotes; write a quote inside one as ''.
...
To keep recursive predicate processing bounded, each WHERE or HAVING expression is independently limited to 64 levels of parenthesis nesting and 256 total comparison/boolean AST nodes. Queries over either limit return a SQL error before execution.
