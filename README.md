...
then synchronizes the parent directory. On non-Solaris Unix, an advisory lock
associated with the opened directory serializes calls to RustHouse replacement
and repair APIs. Linux NFS and other filesystems that return `EBADF` when an
...
failures before publication and separately report post-publication
directory-sync uncertainty. Ordinary replacement is not exposed on Windows;
repair and its locking protocol are not exposed on Windows or Solaris.
