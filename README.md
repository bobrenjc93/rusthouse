...
gauges and the sparse-index measurements are Prometheus counters. That endpoint
additionally exposes the configured aggregate worker cap, which is not a
`system.metrics` row.
...
`CAST(nullable_int64_column AS Int64)`,
`CAST(nullable_int64_column AS Float64)`,
`CAST(nullable_int64_column AS Bool)`, and
`CAST(nullable_int64_column AS String)` are supported. The `Int64` target is an
identity conversion for present values; the other targets use their normal
conversion. For Bool, zero becomes `false` and every nonzero value becomes
`true`, while String uses canonical base-10 integer text. Absent values produce
a typed `NULL` of the target type and add no generated String payload bytes.
...
the bounds drops the pruning metadata; later SELECTs safely fall back to a
complete scan. This is local metadata for the in-memory, single-process,
single-node engine. It is not SQL `PARTITION BY`, distributed partition
routing, sharding, replication, or a durable partition format.
...
body, `/?query=INSERT+INTO+<table>+FORMAT+TabSeparated` with a headerless TSV
body, or `/?query=INSERT+INTO+<table>+FORMAT+TabSeparatedWithNames` with a named
TSV body. Headerless inputs use physical schema order. Each requires one decimal
...
`max_group_key_bytes` parameter, one optional decimal
...
response-byte limit.
`max_rows_to_read` also accepts ASCII decimal digits. A nonzero value tightens
the database's configured source-scan row limit for that request, while zero
...
value tightens the configured estimated group-key byte limit only for that
request, while a larger value never relaxes the configured limit. The effective
cap is checked alongside `max_group_key_cells` before retaining each new key for
`GROUP BY`, `DISTINCT`, and `UNION DISTINCT`, including temporary wide-key probe
storage. String keys borrow their UTF-8 payloads from existing storage, so the
charge covers retained key value references rather than the borrowed payloads.
It does not change `SHOW SETTINGS`, `system.settings`, or persistent database
configuration.
...
request, while a larger value never relaxes the configured limit. The effective
cap applies to both fixed aggregate-state allocation and dynamically cloned
String extrema. It does not change `SHOW SETTINGS`, `system.settings`, or the
persistent database configuration.
...
`max_group_key_cells`, `max_group_key_bytes`, `max_ordering_state_bytes`,
...
family, including the combined parameterized-workload-limit path. A POST through
...
order. Exact `X-ClickHouse-Format: TabSeparated` likewise selects headerless
TSV in physical schema order and treats every physical line as data.
`X-ClickHouse-Format: CSVWithNames` may select named CSV explicitly, while
...
CSV, headerless TSV, and named TSV importers, respectively, with the same
success response, authentication, admission, format-specific resource limits,
and all-or-nothing behavior. Empty named CSV and named TSV bodies fail as a
missing header; empty headerless CSV and TSV bodies are successful zero-row
inserts.
...
and format header validation, SQL/result/scan resource limits, and the complete
HTTP response limit retain their documented ordering and behavior.
...
and retained scalar payload bytes. These are followed by the unlabeled
`rusthouse_global_aggregate_worker_cap` gauge, which reports the database's
current configured computation-lane cap for supported aggregate queries. A
request-local `max_threads` value can only tighten one query and never changes
this gauge. The same response then includes the unlabeled
`rusthouse_index_scanned_blocks` and `rusthouse_index_pruned_blocks` counters
...
total, worker-cap, and counter values, and the complete HTTP envelope before any
name is cloned or sorted. The byte gauges count each `Int64` and `Float64` value
as 8 bytes, each `Bool` as 1 byte, and each `String` by its UTF-8 payload length;
they exclude container capacity, schema text, and allocation metadata and
saturate at the platform's maximum `usize`. The response uses Prometheus text
format version 0.0.4. Table and database totals are maintained during mutations,
so a scrape reads cached measurements instead of scanning retained values. When
the preflight succeeds, capturing the per-table samples copies and sorts the
current catalog names, row counts, and cached byte counts under the same
database read-lock attempt that snapshots the totals, aggregate worker cap, and
index counters, then releases the lock before formatting or writing the
...
contents in plaintext unless the embedding places the bound listener behind
TLS; do not expose them directly to an untrusted network. The single-exchange
functions remain available when an embedding needs a different connection,
concurrency, transport-security, timeout, or shutdown policy.
...
`Nullable(Int64)`, and `Float64` tables. Each physical LF or CRLF record must
be a complete one-element JSON array such as `[-7]`, `[6.022e23]`, or
`[null]`; JSON whitespace around the array and value is accepted. Int64 targets
retain the requirement that values use JSON integer syntax and fit exactly in
`Int64`. Float64 targets accept JSON integer, decimal, and exponent forms only
when they parse to a finite `Float64`, including the finite extrema,
subnormals, and signed zero. Only nullable Int64 targets accept `null`. Empty
input appends no rows, while empty arrays, wider arrays, booleans, strings,
objects, nested arrays, trailing content, malformed JSON, invalid UTF-8,
out-of-range integers, and numbers outside the finite Float64 range are
rejected.
...
table's remaining row and cell capacity before using any applicable Int64 WAL
...
compatible numeric column, with finite Float64 values preserving their exact
binary value (including the sign of zero). `SharedDatabase::ingest_json_compact_each_row`
holds one write lock from lookup through commit; the nonblocking
