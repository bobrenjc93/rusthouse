...
Unix-only restore counterpart. It makes exactly one nonblocking write-lock
attempt before accessing the source file, then delegates bounded decoding,
validation, and atomic registration to `Database::restore_int64_table_from_file`
while retaining that guard. Reader or writer contention, lock poisoning, and
the existing typed snapshot restore failures remain distinct. Every failure
leaves catalog data and cached metrics unchanged.
