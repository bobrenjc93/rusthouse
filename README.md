...
contain commas or embedded LF and CRLF line endings, and doubled quotes inside
it decode to one quote (for example, `"say ""hello"""`). Record boundaries are
recognized only outside quotes. Headers and non-`String` fields must remain
unquoted, and malformed quoting is rejected. Any input, schema, value, limit,
