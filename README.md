...
`AVG(physical_non_nullable_int64_column)` aggregate. Nullable grouped COUNT
partials track group row presence separately from present values, so all-NULL
groups remain visible with a zero count. Bool-grouped SUM/AVG partials maintain a
checked i128 sum and checked row count for each key. SUM applies the normal
checked Int64 final conversion, while AVG finalizes each nonempty group as
Float64. Ordered partition reduction preserves
...
non-nullable column, nullable or non-Int64 `SUM` or `AVG`, more than one
grouping column, non-Bool keys, or multiple aggregates), and
...
integer `COUNT`, or non-nullable Int64 `SUM` and `AVG` shapes. The
