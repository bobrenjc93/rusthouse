...
`--format` accepts `table`, `csv`, or `json`. CSV output follows conventional quoting and represents `NULL` as an empty field. Run `cargo run -- --help` for resource-limit flags.
