...
aggregate, or exactly one `SUM(physical_non_nullable_int64_column)` aggregate.
...
`SUM`, more than one grouping column, non-Bool keys, or multiple aggregates), and
...
parallel global aggregates and the narrow Bool-grouped row count, physical
non-nullable column count, nullable integer `COUNT`, or non-nullable Int64
`SUM` shapes. The
