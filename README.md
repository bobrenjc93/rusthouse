...
failure changes catalog data or cached metrics. One set may mix `Int64` and
`Nullable(Int64)` snapshots while preserving each table's row order and exact
NULL positions.
...
unchanged. The backup and replacement APIs retain their existing non-nullable
boundary.
...
The slice may mix `Int64` and `Nullable(Int64)` snapshots; nullability, exact
NULL positions, row order, and persisted row caps are preserved per entry.
Success makes the complete subset queryable and updates cached metrics; failure
reports the zero-based entry and caller table name while preserving all prior
catalog data and gauges.
