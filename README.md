...
`ROW_NUMBER() OVER (PARTITION BY int64_column)` instead retains source order and
assigns an independent counter to each partition after `WHERE` and before
`LIMIT`; tracked partitions use the grouped-query group and key caps. The empty
window retains source order. These minimal window forms deliberately reject
arguments, partition ordering, multiple partition or sort keys, conflicting
