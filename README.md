...
- explicit BEGIN, COMMIT, and ROLLBACK transactions
...
Database parses a complete SQL batch before execution: any syntax error leaves the catalog unchanged. After parsing succeeds, statements execute in order; if a later execution error occurs, earlier successful statements remain applied. Use BEGIN and COMMIT or ROLLBACK for an explicit atomicity boundary, including across multiple `execute` calls. Reads in a transaction see its staged writes.
...
RustHouse has no NULL, joins, arithmetic expressions, updates, deletes, quoted identifiers, persistence, HTTP API, or network protocol. Data exists only for the lifetime of the Database value or CLI process. A multi-row INSERT is validated in full before any of its rows are appended.
