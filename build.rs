...
fn attested_build_token() -> String {
    ...
    let requested = env::var("RUSTHOUSE_ATTESTED_BUILD").ok();
    let wrapper = env::var("RUSTC_WORKSPACE_WRAPPER").ok();
    match (requested.as_deref(), wrapper.as_deref()) {
        (Some("1"), Some(wrapper)) if !wrapper.trim().is_empty() => fresh_attestation_token(),
        (None, _) => "unavailable".to_owned(),
        (Some(value), _) => {
            panic!(
                "RUSTHOUSE_ATTESTED_BUILD={value:?} requires value 1 and a nonempty RUSTC_WORKSPACE_WRAPPER"
            )
        }
    }
}
