//! In-memory ownership and `CREATE TABLE` execution.

use std::collections::HashMap;
use std::error::Error;
use std::fmt;

use crate::sql::{CreateTableStatement, ParseError, ParseLimits, parse_create_table_with_limits};
...
impl CatalogLimits {
    /// Creates a complete set of catalog limits.
    #[must_use]
...
/// A deterministic failure from catalog lookup or `CREATE TABLE` execution.
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum CatalogError {
    /// SQL could not be parsed as the supported `CREATE TABLE` syntax.
    Parse(ParseError),
...
impl Error for CatalogError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            ...
            Self::TableConstruction { source, .. } => Some(source),
            _ => None,
        }
    }
}
