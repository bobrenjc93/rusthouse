...
///
/// `max_rows` limits both hash-build rows and joined or filtered output rows.
...
impl<'a> QueryInput<'a> {
    ...
    fn apply_join(&mut self, catalog: &'a Catalog, join: &Join, limits: JoinLimits) -> Result<()> {
        ...
        let build_left = !join.kind.is_left_filter() && old_row_count <= right_row_count;
        let build_rows = if join.kind.is_left_filter() {
            if old_row_count == 0 {
                0
            } else {
                right_row_count
        ...
        if join.kind.is_left_filter() {
            let (filtered_rows, filtered_row_count) = self.execute_left_filter_join(
                join.kind,
                old_rows.as_deref(),
                old_width,
                old_row_count,
            ...
            self.columns.truncate(old_column_count);
            self.relations.truncate(old_width);
            self.row_count = filtered_row_count;
            self.rows = Some(filtered_rows);
            return Ok(());
        }
    ...
    fn execute_left_filter_join(
        ...
        debug_assert!(kind.is_left_filter());
        if old_row_count == 0 || right_row_count == 0 {
            let output_count = if kind == JoinKind::LeftAnti {
                old_row_count
            } else {
                0
            };
            let output_indices = checked_filter_join_output_layout(
                output_count,
                old_width,
                resident_bytes,
                0,
            ...
            let mut filtered = Vec::with_capacity(output_indices);
            if kind == JoinKind::LeftAnti {
                for left_row in 0..old_row_count {
                    append_left_row(&mut filtered, old_rows, old_width, left_row);
                }
            }
            return Ok((filtered, output_count));
        }

        let layout = JoinHashLayout::new(right_row_count, keys.len(), limits)?;
        let matched_bytes = std::mem::size_of::<Vec<u8>>()
            .checked_add(checked_allocation_bytes::<u8>(old_row_count, limits)?)
            .ok_or_else(|| join_byte_limit_error(limits, usize::MAX))?;
        let working_resident_bytes = resident_bytes
            .checked_add(matched_bytes)
            .ok_or_else(|| join_byte_limit_error(limits, usize::MAX))?;
        checked_join_bytes(working_resident_bytes, layout.estimated_bytes, 0, limits)?;
        ...
        let mut matched_left = vec![0_u8; old_row_count];
        let mut candidate_count = 0usize;
        for (left_row, matched) in matched_left.iter_mut().enumerate() {
            if !self.old_join_key(old_rows, old_width, keys, left_row, &mut scratch) {
                continue;
            }
            let Some(entry) = hash.get(&scratch) else {
                continue;
            };
            let mut right_row = entry.first_row;
            while right_row != NO_JOIN_INDEX {
        ...
        let keep_matches = kind == JoinKind::LeftSemi;
        let output_count = matched_left
            .iter()
            .filter(|matched| (**matched != 0) == keep_matches)
            .count();
        let output_indices = checked_filter_join_output_layout(
            output_count,
            old_width,
        ...
        let mut filtered = Vec::with_capacity(output_indices);
        for (left_row, matched) in matched_left.iter().enumerate() {
            if (*matched != 0) == keep_matches {
                append_left_row(&mut filtered, old_rows, old_width, left_row);
            }
        }
        debug_assert_eq!(filtered.len(), output_indices);
        Ok((filtered, output_count))
    }
...
fn checked_filter_join_output_layout(
    output_count: usize,
    old_width: usize,
    resident_bytes: usize,
    matched_bytes: usize,
    ...
    let overflow = || join_byte_limit_error(limits, usize::MAX);
    let output_indices = output_count.checked_mul(old_width).ok_or_else(overflow)?;
    let output_bytes = std::mem::size_of::<Vec<usize>>()
        .checked_add(checked_allocation_bytes::<usize>(output_indices, limits)?)
        .ok_or_else(overflow)?;
    let base_bytes = resident_bytes
        .checked_add(matched_bytes)
        .ok_or_else(overflow)?;
    let peak_bytes = base_bytes
        .checked_add(hash_bytes)
        .ok_or_else(overflow)?
        .max(base_bytes.checked_add(output_bytes).ok_or_else(overflow)?);
    if peak_bytes > limits.max_bytes {
        return Err(Error::JoinLimitExceeded {
            resource: "bytes",
            limit: limits.max_bytes,
...
fn append_left_row(
    ...
    if let Some(old_rows) = old_rows {
        let start = left_row * old_width;
        filtered.extend_from_slice(&old_rows[start..start + old_width]);
    } else {
        filtered.push(left_row);
    }
}
