...
fn render_json_compact_each_row(result: &QueryResult) -> String {
    ...
    write_json_compact_each_row(&mut output, result)
        .expect("writing JSONCompactEachRow to a Vec cannot fail");
    String::from_utf8(output).expect("JSONCompactEachRow rendering preserves UTF-8")
}

/// Streams one positional JSON array per result row.
///
/// Column names are omitted, SQL `NULL` is emitted as JSON `null`, and native
/// numbers, booleans, and strings retain their JSON scalar types. Every row is
/// terminated by a line feed; an empty result emits no bytes.
pub fn write_json_compact_each_row(
    output: &mut impl io::Write,
    result: &QueryResult,
) -> io::Result<()> {
    for row in &result.rows {
...
mod tests {
    ...
    #[test]
    fn streams_json_compact_each_row_with_all_types_and_escaping() {
        let result = QueryResult {
            columns: vec![
                ResultColumn {
                    name: "missing".to_owned(),
        ...
        let expected = concat!(
            "[null,-9223372036854775808,2.0,false,\"\\\"\\\\\\b\\f\\n\\r\\t\\u0001雪\"]\n",
            "[\"present\",7,-1.25,true,\"\"]\n",
        );
        let mut output = Vec::new();

        write_json_compact_each_row(&mut output, &result)
            .expect("Vec accepts streamed JSONCompactEachRow");

        assert_eq!(output, expected.as_bytes());
        assert_eq!(render(&result, OutputFormat::JsonCompactEachRow), expected);
    }

    #[test]
    fn json_compact_each_row_empty_result_emits_nothing() {
        let result = QueryResult {
            columns: vec![ResultColumn {
                name: "empty".to_owned(),
                data_type: DataType::Int64,
        ...
        write_json_compact_each_row(&mut output, &result)
            .expect("Vec accepts empty JSONCompactEachRow output");

        assert!(output.is_empty());
        assert_eq!(
            render(&result, OutputFormat::JsonCompactEachRow),
            String::new()
        );
    }
