                ...
                .map(|item| {
                    Ok(match item {
                        ...
                        ResolvedItem::StringLength { source } => Value::Int64(
                            scalar_string::byte_len_to_i64(string_at(table, *source, *row).len())?,
                        ),
                        ResolvedItem::StringLengthUtf8 { source } => Value::Int64(
                            scalar_string::scalar_len_to_i64(string_at(table, *source, *row))?,
                        ),
    ...
    sort_and_limit(rows, limit, |left, right| {
        for order in ordering {
            let comparison = match items[order.output] {
                ...
                ResolvedItem::StringLower { source } => scalar_string::lower_cmp(
                    string_at(table, source, left),
                    string_at(table, source, right),
                ),
                ResolvedItem::StringUpper { source } => scalar_string::upper_cmp(
                    string_at(table, source, left),
                    string_at(table, source, right),
                ),
