...
use crate::batch::storage::{Column, Int64RangeFilter, Table, validate_table_name};
...
pub use crate::batch::storage::{
    DEFAULT_MAX_CELLS_PER_TABLE, DEFAULT_MAX_COLUMNS_PER_TABLE,
    DEFAULT_MAX_INT64_RANGE_PARTITION_BYTES, DEFAULT_MAX_INT64_RANGE_PARTITION_ROWS,
    DEFAULT_MAX_INT64_RANGE_PARTITIONS, DEFAULT_MAX_ROWS_PER_TABLE, Int64RangePartition,
...
impl Database {
    ...
    fn execute_select_with_prefix(
        ...
        // Ordinary tables and unsupported predicates retain the complete-scan
        // path. A direct Int64 key comparison may first discard impossible
        // validated partitions; the exact compiled predicate still evaluates
        // every admitted row before any later SELECT operator runs.
        let source_rows = predicate
            .as_ref()
            .and_then(CompiledPredicate::int64_range_filter)
        ...
        let mut matching_rows = if window_ordering.is_some() {
            ...
            let matching_row_count = source_rows.clone().filter(|row| row_matches(*row)).count();
            ...
            rows.extend(source_rows.clone().filter(|row| row_matches(*row)));
...
fn paired_global_count_aggregate(
    ...
    let count_state = aggregate_specs.iter().position(|spec| {
        spec.function == AggregateFunction::Count
            && spec.argument.is_none()
...
impl CompiledPredicate {
    fn int64_range_filter(&self) -> Option<(usize, Int64RangeFilter)> {
        let Self::Comparison {
            left,
            operator,
            right,
        ...
        let filter = match operator {
            ComparisonOperator::Equal => Int64RangeFilter::Equal(value),
            ComparisonOperator::Less => Int64RangeFilter::Less(value),
            ComparisonOperator::LessOrEqual => Int64RangeFilter::LessOrEqual(value),
            ComparisonOperator::Greater => Int64RangeFilter::Greater(value),
            ComparisonOperator::GreaterOrEqual => Int64RangeFilter::GreaterOrEqual(value),
...
mod tests {
    ...
    #[test]
    fn global_max_float64_forced_workers_match_empty_null_having_and_pagination() {
        let mut database = max_float64_database(0, |_| unreachable!("empty input"));
        let first_page = assert_global_aggregate_worker_differential(
            &mut database,
            "SELECT MAX(value) AS maximum FROM empty_float_max_values \
    ...
    #[test]
    fn global_max_float64_forced_workers_preserve_filtering_extrema_and_first_signed_zero() {
        let row_count = GLOBAL_AGGREGATE_PARALLEL_ROW_THRESHOLD + 3;
        ...
        let sql = "SELECT MAX(value) AS maximum FROM float_values_to_max \
        ...
        let Value::Float64(single_maximum) = &single_worker.rows[0][0] else {
            panic!("single-worker maximum must be Float64")
        };
        let Value::Float64(multi_maximum) = &multi_worker.rows[0][0] else {
        ...
        let finite_extreme = assert_global_aggregate_worker_differential(
            &mut database,
            "SELECT MAX(value) FROM float_values_to_max",
        );
        assert_eq!(finite_extreme.rows, [vec![Value::Float64(f64::MAX)]]);
    }
    ...
    fn global_max_float64_parallel_boundary_uses_shared_budget_only_for_the_sole_global_shape() {
        ...
        for matched_rows in [
            ...
            let result = assert_global_aggregate_worker_differential(
                &mut database,
                &format!("SELECT MAX(value) FROM float_values_to_max WHERE id <= {matched_rows}"),
            );
    ...
    #[test]
    fn global_max_float64_worker_failure_repeats_the_complete_maximum_locally() {
