        ...
        }) = paired_global_count
        {
            ...
            let count = match count_source {
                ...
                PairedGlobalCountSource::AggregatePresentValues => count_present_values(
                    present_count.expect("same-column nullable AVG exposes a present count"),
                )?,
            };
...
fn execute_grouped_bool_count<'a>(
    ...
    // `None` counts every matching row; `Some` supplies the only admitted
    // nullable physical argument and excludes its absent values.
    let count_values = match spec {
        AggregateSpec {
            function: AggregateFunction::Count,
            argument: None,
            input_type: None,
        ...
        AggregateSpec {
            ...
            let Column::NullableInt64(values) = &table.columns()[*argument] else {
                return Ok(None);
            };
    ...
    let partial = reduce_grouped_bool_count(matching_rows, parallelism, |rows| {
        grouped_bool_count_chunk(group_values, count_values, rows)
    })?;
    let group_count = usize::from(partial.false_seen) + usize::from(partial.true_seen);
...
where
    C: Fn(&[usize]) -> Result<GroupedBoolCountPartial> + Sync,
{
    ...
    let parallel_result =
        try_parallel_grouped_bool_count(matching_rows, admission.helper_threads(), &chunk);
    drop(admission);
    parallel_result.unwrap_or_else(|| chunk(matching_rows))
}

fn try_parallel_grouped_bool_count<C>(
    matching_rows: &[usize],
    helper_threads: usize,
    chunk: &C,
) -> Option<Result<GroupedBoolCountPartial>>
where
    C: Fn(&[usize]) -> Result<GroupedBoolCountPartial> + Sync,
{
    ...
    std::thread::scope(|scope| {
        ...
        for chunk_index in 1..worker_count {
            ...
            let spawn = std::thread::Builder::new()
                .name(format!("rusthouse-group-bool-count-{chunk_index}"))
        ...
        Some(
            partials
                .into_iter()
                .collect::<Result<Vec<_>>>()
                .and_then(reduce_grouped_bool_count_partials),
        )
    })
}
...
fn grouped_bool_count_chunk(
    group_values: &[bool],
    count_values: Option<&[Option<i64>]>,
    matching_rows: &[usize],
) -> Result<GroupedBoolCountPartial> {
    let mut partial = GroupedBoolCountPartial::default();
    for row in matching_rows {
        let value = group_values[*row];
        partial.first_seen.get_or_insert(value);
        // Membership is independent of the nullable count so an all-NULL key
        // still produces a group with a finalized zero.
        if value {
            partial.true_seen = true;
        } else {
            partial.false_seen = true;
        }
        if count_values.is_some_and(|values| values[*row].is_none()) {
            continue;
        }
        let count = if value {
...
fn reduce_grouped_bool_count_partials(
    ...
    partials
        ...
        .try_fold(GroupedBoolCountPartial::default(), |mut total, partial| {
            ...
            total.false_seen |= partial.false_seen;
            total.true_seen |= partial.true_seen;
...
mod tests {
    ...
    fn grouped_bool_nullable_count_database(
        ...
        database
            .execute(
                "CREATE TABLE nullable_bool_events (id Int64, active Bool); \
                 ALTER TABLE nullable_bool_events ADD COLUMN reading Nullable(Int64);",
            )
        ...
        if row_count > 0 {
            for first_id in (1..=row_count).step_by(50_000) {
                ...
                let rows = (first_id..=last_id)
                    .map(|id| {
                        let (active, reading) = row_values(id);
                        let reading =
                            reading.map_or_else(|| "NULL".to_owned(), |value| value.to_string());
                        format!("({id}, {active}, {reading})")
                    })
                    .collect::<Vec<_>>()
                    .join(",");
                database
                    .execute(&format!("INSERT INTO nullable_bool_events VALUES {rows}"))
    ...
    #[test]
    fn grouped_bool_nullable_count_crosses_threshold_and_uses_shared_admission_only_for_its_shape()
    {
        static BUDGET: GlobalAggregateWorkerBudget = GlobalAggregateWorkerBudget::for_test(3);

        let row_count = GLOBAL_AGGREGATE_PARALLEL_ROW_THRESHOLD + 2;
        let mut database = grouped_bool_nullable_count_database(row_count, |id| {
            (id % 2 == 1, (id % 3 != 0).then_some(7))
        });
        let boundary_sql = format!(
            "SELECT active, COUNT(reading) FROM nullable_bool_events \
             WHERE id <= {} GROUP BY active",
            GLOBAL_AGGREGATE_PARALLEL_ROW_THRESHOLD
        );
        let above_threshold_sql =
            "SELECT active, COUNT(reading) FROM nullable_bool_events GROUP BY active";
        let boundary_expected =
            assert_global_aggregate_worker_differential(&mut database, &boundary_sql);
        let expected =
            assert_global_aggregate_worker_differential(&mut database, above_threshold_sql);
        ...
        BUDGET.reset_peak();
        let boundary = query(&mut database, &boundary_sql);
        assert_eq!(boundary, boundary_expected);
        assert_eq!(BUDGET.peak_helpers_in_use(), 0);

        let above_threshold = query(&mut database, above_threshold_sql);
        assert_eq!(above_threshold, expected);
        assert!(BUDGET.peak_helpers_in_use() > 0);
        assert_eq!(BUDGET.helpers_in_use(), 0);

        for unsupported in [
            "SELECT active, COUNT(id) FROM nullable_bool_events GROUP BY active",
            "SELECT active, COUNT(reading), COUNT(*) FROM nullable_bool_events GROUP BY active",
            "SELECT reading, COUNT(reading) FROM nullable_bool_events GROUP BY reading",
        ] {
            BUDGET.reset_peak();
            assert!(!query(&mut database, unsupported).rows.is_empty());
            assert_eq!(
                BUDGET.peak_helpers_in_use(),
                0,
                "unsupported grouped shape stays sequential: {unsupported}"
            );
        }
        ...
        let held = BUDGET
            .acquire_for_test(BUDGET.helper_limit())
            .expect("test exhausts aggregate helper admission");
        let exhausted = query(&mut database, above_threshold_sql);
    ...
    #[test]
    fn grouped_bool_nullable_count_workers_preserve_filtered_all_null_groups_having_and_ordering() {
        let row_count = GLOBAL_AGGREGATE_PARALLEL_ROW_THRESHOLD + 5;
        let mut database =
            grouped_bool_nullable_count_database(row_count, |id| (id % 2 == 1, None));

        let filtered = assert_global_aggregate_worker_differential(
            &mut database,
            "SELECT active, COUNT(reading) AS present FROM nullable_bool_events \
             WHERE id > 2 GROUP BY active",
        );
        assert_eq!(
            filtered.rows,
            [
            ...
            "zero-count groups survive the established grouped-key ordering"
        );

        let ordered = assert_global_aggregate_worker_differential(
            &mut database,
            "SELECT active AS enabled, COUNT(reading) AS present \
             FROM nullable_bool_events WHERE id > 2 GROUP BY active \
             HAVING present = 0 ORDER BY enabled ASC LIMIT 1 OFFSET 1",
        );
        assert_eq!(ordered.rows, [vec![Value::Bool(true), Value::Int64(0)]]);
    }

    #[test]
    fn grouped_bool_nullable_count_resource_boundaries_match_sequential_execution() {
        let row_count = GLOBAL_AGGREGATE_PARALLEL_ROW_THRESHOLD + 1;
        let fixed_state_bytes = 2_usize
            .saturating_mul(std::mem::size_of::<AggregateState>())
            .saturating_add(std::mem::size_of::<Vec<AggregateState>>());
        ...
        let mut database = grouped_bool_nullable_count_database(row_count, |id| (id == 1, None));
        database.query_result_limits = exact_limits;
        let sql = "SELECT active, COUNT(reading) FROM nullable_bool_events GROUP BY active";

        let exact = assert_global_aggregate_worker_differential(&mut database, sql);
        assert_eq!(
            exact.rows,
            [
                vec![Value::Bool(false), Value::Int64(0)],
    ...
    fn grouped_bool_count_worker_failure_repeats_complete_grouping_locally() {
        ...
        let successful_parallel = reduce_grouped_bool_count(
            &matching_rows,
            GlobalAggregateParallelism::fixed(2),
            |rows| grouped_bool_count_chunk(&values, None, rows),
        )
        ...
        let partial = reduce_grouped_bool_count(
            ...
            |rows| {
                ...
                grouped_bool_count_chunk(&values, None, rows)
            },
        )
    ...
    #[test]
    fn grouped_bool_nullable_count_worker_failure_repeats_complete_null_aware_grouping_locally() {
        let group_values = [false, true];
        let count_values = [None, Some(9)];
        let row_count = GLOBAL_AGGREGATE_PARALLEL_ROW_THRESHOLD + 1;
        let mut matching_rows = vec![0; row_count];
        let second_partition = parallel_aggregate_partition(&matching_rows, 2, 0).len();
        matching_rows[second_partition] = 1;
        ...
        let successful_parallel = reduce_grouped_bool_count(
            &matching_rows,
            GlobalAggregateParallelism::fixed(2),
            |rows| grouped_bool_count_chunk(&group_values, Some(&count_values), rows),
        )
        .expect("deterministic parallel nullable grouping succeeds");
        ...
        let failed_parallel = reduce_grouped_bool_count(
            ...
            |rows| {
                if std::thread::current().name() == Some("rusthouse-group-bool-count-1") {
                    panic!("injected grouped Bool nullable COUNT worker failure");
                }
                grouped_bool_count_chunk(&group_values, Some(&count_values), rows)
            },
        )
        .expect("worker failure falls back to complete local nullable grouping");
        ...
        assert_eq!(
            reduce_grouped_bool_count_partials(vec![
                GroupedBoolCountPartial {
                    true_count: i64::MAX,
                    true_seen: true,
