...
fn execute_grouped<'a>(
    ...
    if let Some(grouped) = execute_grouped_bool_min(
        table,
        matching_rows,
        group_columns,
        aggregate_specs,
...
fn execute_grouped_bool_count<'a>(
    ...
    let partial = match spec.function {
        AggregateFunction::Count => match (spec.argument, spec.input_type) {
            (None, None) => {
                grouped_bool_count::reduce_rows(group_values, matching_rows, parallelism)?
            }
            ...
            (Some(argument), Some(DataType::Int64)) if table.column_is_nullable_int64(argument) => {
                ...
                grouped_bool_count::reduce_nullable_int64(
                    group_values,
                    count_values,
                    matching_rows,
                    parallelism,
        ...
        AggregateFunction::CountIf => {
            let (Some(argument), Some(DataType::Bool)) = (spec.argument, spec.input_type) else {
                return Ok(None);
            };
            let Column::Bool(count_values) = &table.columns()[argument] else {
                return Ok(None);
            };
            grouped_bool_count::reduce_count_if(
                group_values,
                count_values,
                matching_rows,
                parallelism,
...
fn execute_grouped_bool_sum_or_avg<'a>(
    ...
    let operation = match function {
        AggregateFunction::Sum => grouped_bool_sum_avg::Operation::Sum,
        AggregateFunction::Avg => grouped_bool_sum_avg::Operation::Avg,
        _ => unreachable!("grouped Bool SUM/AVG shape is resolved"),
    };
    let partial =
        grouped_bool_sum_avg::reduce(group_values, values, matching_rows, parallelism, operation)?;
    ...
    if let Some(first) = partial.first_seen() {
        keys.push(GroupKey::One(ValueRef::Bool(first)));
        aggregates.push(finished_value(first)?);
        let second = !first;
        if partial.present(second) {
            keys.push(GroupKey::One(ValueRef::Bool(second)));
            aggregates.push(finished_value(second)?);
        }
    }
    debug_assert_eq!(keys.len(), group_count);
...
fn execute_grouped_bool_min<'a>(
    ...
    match &table.columns()[*value_column] {
        Column::Int64(values) => finish_grouped_bool_min(
            grouped_bool_min::reduce_int64(group_values, values, matching_rows, parallelism)?,
            limits,
            Value::Int64,
        ),
        Column::Float64(values) => finish_grouped_bool_min(
...
fn finish_grouped_bool_min<'a, T, F>(
    partial: grouped_bool_min::Partial<T>,
    limits: QueryResultLimits,
    into_value: F,
) -> Result<Option<GroupedData<'a>>>
where
    ...
    F: Fn(T) -> Value,
{
    ...
    if let Some(first) = partial.first_seen() {
        keys.push(GroupKey::One(ValueRef::Bool(first)));
        minimums.push(into_value(partial.minimum(first)));
        let second = !first;
        if partial.present(second) {
            keys.push(GroupKey::One(ValueRef::Bool(second)));
            minimums.push(into_value(partial.minimum(second)));
        }
    }
    debug_assert_eq!(keys.len(), group_count);
    debug_assert_eq!(minimums.len(), group_count);
    Ok(Some(GroupedData {
        keys,
...
fn execute_grouped_bool_max<'a>(
    ...
    match &table.columns()[*value_column] {
        Column::Int64(values) => finish_grouped_bool_max(
            grouped_bool_max::reduce_int64(group_values, values, matching_rows, parallelism)?,
            limits,
            Value::Int64,
        ),
        Column::Float64(values) => finish_grouped_bool_max(
            grouped_bool_max::reduce_float64(group_values, values, matching_rows, parallelism)?,
            limits,
            Value::Float64,
        ),
        _ => Ok(None),
...
fn finish_grouped_bool_max<'a, T, F>(
    partial: grouped_bool_max::Partial<T>,
    limits: QueryResultLimits,
    into_value: F,
) -> Result<Option<GroupedData<'a>>>
where
    ...
    F: Fn(T) -> Value,
{
    ...
    if let Some(first) = partial.first_seen() {
        keys.push(GroupKey::One(ValueRef::Bool(first)));
        maximums.push(into_value(partial.maximum(first)));
        let second = !first;
        if partial.present(second) {
            keys.push(GroupKey::One(ValueRef::Bool(second)));
            maximums.push(into_value(partial.maximum(second)));
        }
    }
    debug_assert_eq!(keys.len(), group_count);
...
fn min_global_int64(
    ...
    let minimum = match &table.columns()[spec.argument.expect("MIN argument")] {
        Column::Int64(values) => {
            global_scalar_extremum::min_int64(values, matching_rows, parallelism)
        }
        Column::NullableInt64(values) => {
            global_scalar_extremum::min_nullable_int64(values, matching_rows, parallelism)
        }
...
fn min_global_float64(
    ...
    AggregateState::Min(
        global_scalar_extremum::min_float64(values, matching_rows, parallelism).map(Value::Float64),
    )
}
...
fn max_global_int64(
    ...
    let maximum = match &table.columns()[spec.argument.expect("MAX argument")] {
        Column::Int64(values) => {
            global_scalar_extremum::max_int64(values, matching_rows, parallelism)
        }
        Column::NullableInt64(values) => {
            global_scalar_extremum::max_nullable_int64(values, matching_rows, parallelism)
        }
...
mod tests {
    ...
    fn grouped_bool_min_database(
        ...
        database
            .execute(
                "CREATE TABLE bool_min_events \
                 (id Int64, value Int64, active Bool, included Bool);",
            )
            .expect("grouped Bool MIN differential setup");
        if row_count > 0 {
            for first_id in (1..=row_count).step_by(50_000) {
                let last_id = first_id.saturating_add(49_999).min(row_count);
                let rows = (first_id..=last_id)
                    ...
                    .join(",");
                database
                    .execute(&format!("INSERT INTO bool_min_events VALUES {rows}"))
                    .expect("grouped Bool MIN differential rows");
            }
        }
        database
    }

    fn grouped_bool_float64_min_database(
        row_count: usize,
        row_values: impl Fn(usize) -> (f64, bool, bool),
    ) -> Database {
        let mut database = Database::new();
        database
            .execute(
                "CREATE TABLE bool_float_min_events \
                 (id Int64, value Float64, active Bool, included Bool);",
            )
            .expect("grouped Bool Float64 MIN differential setup");
        if row_count > 0 {
            for first_id in (1..=row_count).step_by(50_000) {
                let last_id = first_id.saturating_add(49_999).min(row_count);
                let rows = (first_id..=last_id)
                    ...
                    .join(",");
                database
                    .execute(&format!("INSERT INTO bool_float_min_events VALUES {rows}"))
                    .expect("grouped Bool Float64 MIN differential rows");
            }
        }
        database
    }

    fn grouped_bool_float64_max_database(
        row_count: usize,
        row_values: impl Fn(usize) -> (f64, bool, bool),
    ) -> Database {
        let mut database = Database::new();
        database
            .execute(
                "CREATE TABLE bool_float_max_events \
                 (id Int64, value Float64, active Bool, included Bool);",
            )
            .expect("grouped Bool Float64 MAX differential setup");
        if row_count > 0 {
            for first_id in (1..=row_count).step_by(50_000) {
                let last_id = first_id.saturating_add(49_999).min(row_count);
                let rows = (first_id..=last_id)
                    ...
                    .join(",");
                database
                    .execute(&format!("INSERT INTO bool_float_max_events VALUES {rows}"))
                    .expect("grouped Bool Float64 MAX differential rows");
            }
        }
        database
    }
    ...
    #[test]
    fn grouped_bool_sum_and_avg_threshold_filter_extrema_and_admission_are_differential() {
        static BUDGET: GlobalAggregateWorkerBudget = GlobalAggregateWorkerBudget::for_test(3);
        ...
        let filtered_avg_sql = "SELECT active AS enabled, AVG(value) AS mean \
                                FROM bool_sum_events WHERE included = true GROUP BY active \
                                HAVING mean < 0";
        let expected_mean = -1.0 / ((row_count - 2) / 2) as f64;
        assert_eq!(
            assert_global_aggregate_worker_differential(&mut database, filtered_avg_sql).rows,
            [
                vec![Value::Bool(false), Value::Float64(expected_mean)],
                vec![Value::Bool(true), Value::Float64(expected_mean)],
            ],
            "grouped AVG retains exact Int64 extrema through checked sum-and-count partials"
        );
        assert_eq!(
            assert_global_aggregate_worker_differential(
                &mut database,
                &format!("{filtered_avg_sql} ORDER BY mean DESC, enabled DESC LIMIT 1 OFFSET 1"),
            )
            .rows,
            [vec![Value::Bool(false), Value::Float64(expected_mean)]],
            "grouped AVG remains upstream of HAVING, stable ordering, and pagination"
        );
        ...
        let sequential = force_global_aggregate_workers(&mut database, 1, filtered_avg_sql);
        database.global_aggregate_parallelism =
            GlobalAggregateParallelism::budgeted(NonZeroUsize::new(4).unwrap(), &BUDGET);
        let held = BUDGET
            .acquire_for_test(BUDGET.helper_limit())
            .expect("test exhausts grouped AVG helper admission");
        let exhausted = query(&mut database, filtered_avg_sql);
        assert_eq!(exhausted, sequential);
        assert_eq!(BUDGET.helpers_in_use(), BUDGET.helper_limit());
        drop(held);
        assert_eq!(BUDGET.helpers_in_use(), 0);
    ...
    #[test]
    fn grouped_bool_sum_and_avg_forced_workers_preserve_resource_boundaries() {
        let row_count = GLOBAL_AGGREGATE_PARALLEL_ROW_THRESHOLD + 1;
        ...
        database.query_result_limits = exact_limits;
        let sum_sql = "SELECT active, SUM(value) FROM bool_sum_events GROUP BY active";

        let sequential = force_global_aggregate_workers(&mut database, 1, sum_sql);
        let parallel = force_global_aggregate_workers(&mut database, 4, sum_sql);
        assert_eq!(parallel, sequential);
        assert_eq!(
            parallel.rows,
            [
        ...
        let avg_sql = "SELECT active, AVG(value) FROM bool_sum_events GROUP BY active";
        let sequential = force_global_aggregate_workers(&mut database, 1, avg_sql);
        let parallel = force_global_aggregate_workers(&mut database, 4, avg_sql);
        assert_eq!(parallel, sequential);
        assert_eq!(
            parallel.rows,
            [
                vec![Value::Bool(false), Value::Float64(1.0)],
                vec![Value::Bool(true), Value::Float64(1.0)],
            ]
        );
        ...
        for (limits, expected_error) in [
            ...
            for sql in [sum_sql, avg_sql] {
                ...
                let parallel = database.execute(sql);
                assert_eq!(parallel, sequential);
                assert_eq!(parallel, Err(expected_error.clone()));
            }
        }
    }

    #[test]
    fn grouped_bool_min_threshold_semantics_admission_and_shape_are_differential() {
        static BUDGET: GlobalAggregateWorkerBudget = GlobalAggregateWorkerBudget::for_test(3);
        ...
        let empty = assert_global_aggregate_worker_differential(
            &mut database,
            "SELECT active, MIN(value) FROM bool_min_events \
             WHERE id < 0 GROUP BY active",
        );
        assert!(empty.rows.is_empty());
        ...
        assert_eq!(
            single_group.rows,
            [vec![Value::Bool(false), Value::Int64(i64::MAX)]]
        );

        let filtered_sql = "SELECT active AS enabled, MIN(value) AS minimum \
                            FROM bool_min_events WHERE included = true GROUP BY active";
        assert_eq!(
            assert_global_aggregate_worker_differential(&mut database, filtered_sql).rows,
            [
                vec![Value::Bool(false), Value::Int64(i64::MAX)],
                vec![Value::Bool(true), Value::Int64(i64::MIN)],
            ],
        ...
        assert_eq!(
            assert_global_aggregate_worker_differential(
                &mut database,
                &format!(
                    "{filtered_sql} HAVING minimum <= 9223372036854775807 \
        ...
        assert!(
            BUDGET.peak_helpers_in_use() > 0,
            "a sole Bool-grouped MIN(Int64) above the threshold uses shared helpers"
        );
        assert_eq!(BUDGET.helpers_in_use(), 0);
        ...
        let held = BUDGET
            .acquire_for_test(BUDGET.helper_limit())
            .expect("test exhausts grouped MIN helper admission");
        let exhausted = query(&mut database, filtered_sql);
        assert_eq!(exhausted, sequential);
        assert_eq!(BUDGET.helpers_in_use(), BUDGET.helper_limit());
        drop(held);
        ...
        database
            .execute("ALTER TABLE bool_min_events ADD COLUMN nullable_value Nullable(Int64)")
            .expect("nullable grouped MIN exclusion setup");
        for unsupported_sql in [
            "SELECT active, MIN(value), COUNT(*) FROM bool_min_events GROUP BY active",
    ...
    #[test]
    fn grouped_bool_min_forced_workers_preserve_resource_boundaries() {
        let row_count = GLOBAL_AGGREGATE_PARALLEL_ROW_THRESHOLD + 1;
        let mut database =
            grouped_bool_min_database(row_count, |id| (i64::try_from(id).unwrap(), id == 1, true));
        let aggregate_state_bytes = 2_usize
            .saturating_mul(std::mem::size_of::<AggregateState>())
            .saturating_add(std::mem::size_of::<Vec<AggregateState>>());
        let group_key_bytes = 2_usize.saturating_mul(ESTIMATED_GROUP_KEY_CELL_BYTES);
        ...
        database.query_result_limits = exact_limits;
        let sql = "SELECT active, MIN(value) FROM bool_min_events GROUP BY active";
    ...
    fn grouped_bool_float64_min_is_narrow_parallel_and_sequentially_differential() {
        ...
        let row_count = GLOBAL_AGGREGATE_PARALLEL_ROW_THRESHOLD + 4;
        let mut database = grouped_bool_float64_min_database(row_count, |id| {
            let active = id == 1 || id == row_count;
            let value = if id == 1 {
                -0.0
            } else if id == row_count {
                0.0
            } else if id == 2 || id == row_count - 1 {
                f64::MIN
            } else {
                f64::MAX
            };
        ...
        let empty = assert_global_aggregate_worker_differential(
            &mut database,
            "SELECT active, MIN(value) FROM bool_float_min_events \
             WHERE id < 0 GROUP BY active",
        );
        assert!(empty.rows.is_empty());
        ...
        assert_eq!(
            single_group.rows,
            [vec![Value::Bool(false), Value::Float64(f64::MIN)]]
        );

        let filtered_sql = "SELECT active AS enabled, MIN(value) AS minimum \
                            FROM bool_float_min_events \
                            WHERE included = true GROUP BY active";
        let filtered = assert_global_aggregate_worker_differential(&mut database, filtered_sql);
        assert_eq!(filtered.rows.len(), 2);
        assert_eq!(
            filtered.rows[0],
            [Value::Bool(false), Value::Float64(f64::MIN)]
        );
        assert_eq!(filtered.rows[1][0], Value::Bool(true));
        let Value::Float64(true_minimum) = &filtered.rows[1][1] else {
            panic!("true-group minimum must be Float64")
        };
        assert_eq!(true_minimum.to_bits(), (-0.0_f64).to_bits());

        let page = assert_global_aggregate_worker_differential(
            &mut database,
            &format!(
                "{filtered_sql} HAVING minimum <= 0.0 \
                 ORDER BY minimum ASC, enabled DESC LIMIT 1 OFFSET 1"
            ),
        );
        assert_eq!(page.rows.len(), 1);
        assert_eq!(page.rows[0][0], Value::Bool(true));
        let Value::Float64(page_minimum) = &page.rows[0][1] else {
            panic!("paginated grouped minimum must be Float64")
        };
        assert_eq!(page_minimum.to_bits(), (-0.0_f64).to_bits());
        ...
        assert!(
            BUDGET.peak_helpers_in_use() > 0,
            "a sole Bool-grouped MIN(Float64) above the threshold uses shared helpers"
        );
        assert_eq!(BUDGET.helpers_in_use(), 0);
        ...
        let held = BUDGET
            .acquire_for_test(BUDGET.helper_limit())
            .expect("test exhausts grouped Float64 MIN helper admission");
        let exhausted = query(&mut database, filtered_sql);
        assert_eq!(exhausted, sequential);
        let Value::Float64(exhausted_true_minimum) = &exhausted.rows[1][1] else {
            panic!("fallback grouped minimum must be Float64")
        };
        assert_eq!(exhausted_true_minimum.to_bits(), (-0.0_f64).to_bits());
        assert_eq!(BUDGET.helpers_in_use(), BUDGET.helper_limit());
        drop(held);
        assert_eq!(BUDGET.helpers_in_use(), 0);

        for unsupported_sql in [
            "SELECT active, MIN(value), COUNT(*) FROM bool_float_min_events GROUP BY active",
            "SELECT active, included, MIN(value) FROM bool_float_min_events \
        ...
        database.global_aggregate_parallelism = GlobalAggregateParallelism::fixed(1);
        let sequential_error = database.execute(filtered_sql);
        database.global_aggregate_parallelism = GlobalAggregateParallelism::fixed(4);
        let parallel_error = database.execute(filtered_sql);
    ...
    #[test]
    fn grouped_bool_float64_max_is_narrow_parallel_and_sequentially_differential() {
        static BUDGET: GlobalAggregateWorkerBudget = GlobalAggregateWorkerBudget::for_test(3);

        let row_count = GLOBAL_AGGREGATE_PARALLEL_ROW_THRESHOLD + 4;
        let mut database = grouped_bool_float64_max_database(row_count, |id| {
            let active = id == 1 || id == row_count;
            let value = if id == 1 {
                -0.0
            } else if id == row_count {
        ...
        let empty = assert_global_aggregate_worker_differential(
            &mut database,
            "SELECT active, MAX(value) FROM bool_float_max_events \
             WHERE id < 0 GROUP BY active",
        );
        assert!(empty.rows.is_empty());
        ...
        assert_eq!(
            single_group.rows,
            [vec![Value::Bool(false), Value::Float64(f64::MAX)]]
        );

        let filtered_sql = "SELECT active AS enabled, MAX(value) AS maximum \
                            FROM bool_float_max_events \
                            WHERE included = true GROUP BY active";
        let filtered = assert_global_aggregate_worker_differential(&mut database, filtered_sql);
        assert_eq!(filtered.rows.len(), 2);
        assert_eq!(
            filtered.rows[0],
            [Value::Bool(false), Value::Float64(f64::MAX)]
        );
        assert_eq!(filtered.rows[1][0], Value::Bool(true));
        let Value::Float64(true_maximum) = &filtered.rows[1][1] else {
            panic!("true-group maximum must be Float64")
        };
        assert_eq!(true_maximum.to_bits(), (-0.0_f64).to_bits());

        let page = assert_global_aggregate_worker_differential(
            &mut database,
            &format!(
                "{filtered_sql} HAVING maximum >= 0.0 \
                 ORDER BY maximum DESC, enabled ASC LIMIT 1 OFFSET 1"
            ),
        );
        assert_eq!(page.rows.len(), 1);
        assert_eq!(page.rows[0][0], Value::Bool(true));
        let Value::Float64(page_maximum) = &page.rows[0][1] else {
            panic!("paginated grouped maximum must be Float64")
        };
        assert_eq!(page_maximum.to_bits(), (-0.0_f64).to_bits());
        ...
        assert!(
            BUDGET.peak_helpers_in_use() > 0,
            "a sole Bool-grouped MAX(Float64) above the threshold uses shared helpers"
        );
        assert_eq!(BUDGET.helpers_in_use(), 0);
        ...
        let held = BUDGET
            .acquire_for_test(BUDGET.helper_limit())
            .expect("test exhausts grouped Float64 MAX helper admission");
        let exhausted = query(&mut database, filtered_sql);
        assert_eq!(exhausted, sequential);
        let Value::Float64(exhausted_true_maximum) = &exhausted.rows[1][1] else {
            panic!("fallback grouped maximum must be Float64")
        };
        assert_eq!(exhausted_true_maximum.to_bits(), (-0.0_f64).to_bits());
        assert_eq!(BUDGET.helpers_in_use(), BUDGET.helper_limit());
        drop(held);
        assert_eq!(BUDGET.helpers_in_use(), 0);

        for unsupported_sql in [
            "SELECT active, MAX(value), COUNT(*) FROM bool_float_max_events GROUP BY active",
            "SELECT active, included, MAX(value) FROM bool_float_max_events \
             GROUP BY active, included",
            "SELECT value, MAX(id) FROM bool_float_max_events GROUP BY value",
            "SELECT active, MAX(active) FROM bool_float_max_events GROUP BY active",
        ] {
            BUDGET.reset_peak();
            query(&mut database, unsupported_sql);
            assert_eq!(
    ...
    #[test]
    fn grouped_bool_count_if_threshold_arguments_admission_and_semantics_are_differential() {
        static BUDGET: GlobalAggregateWorkerBudget = GlobalAggregateWorkerBudget::for_test(3);

        let row_count = GLOBAL_AGGREGATE_PARALLEL_ROW_THRESHOLD + 2;
        let mut database = count_if_database(row_count);
        let count_for = |active: bool| {
            i64::try_from(
                (1..=row_count)
                    .filter(|id| (*id % 2 == 0) == active && *id % 3 != 0)
                    .count(),
            )
            .unwrap()
        };

        let different_argument_sql =
            "SELECT active, countIf(included) AS n FROM events GROUP BY active";
        assert_eq!(
            assert_global_aggregate_worker_differential(&mut database, different_argument_sql,)
                .rows,
            [
                vec![Value::Bool(false), Value::Int64(count_for(false))],
                vec![Value::Bool(true), Value::Int64(count_for(true))],
            ],
            "a different Bool argument counts true values independently per key"
        );

        let same_argument_sql = "SELECT active AS enabled, countIf(active) AS n FROM events \
             WHERE id > 1 GROUP BY active HAVING n >= 0 \
             ORDER BY n DESC, enabled DESC LIMIT 1 OFFSET 1";
        assert_eq!(
            assert_global_aggregate_worker_differential(&mut database, same_argument_sql).rows,
            [vec![Value::Bool(false), Value::Int64(0)]],
            "the same Bool argument retains a zero-count group through filtering, \
             HAVING, ordering, and pagination"
        );
        ...
        database.global_aggregate_parallelism =
            GlobalAggregateParallelism::budgeted(worker_cap, &BUDGET);
        let boundary_sql = format!(
            "SELECT active, countIf(included) FROM events WHERE id <= {} GROUP BY active",
            GLOBAL_AGGREGATE_PARALLEL_ROW_THRESHOLD
        );
        BUDGET.reset_peak();
        query(&mut database, &boundary_sql);
        assert_eq!(
            BUDGET.peak_helpers_in_use(),
            0,
            "the exact threshold remains sequential"
        );

        let above_threshold_sql = format!(
            "SELECT active, countIf(active) FROM events WHERE id <= {} GROUP BY active",
            GLOBAL_AGGREGATE_PARALLEL_ROW_THRESHOLD + 1
        );
        BUDGET.reset_peak();
        query(&mut database, &above_threshold_sql);
        assert!(
            BUDGET.peak_helpers_in_use() > 0,
            "the sole eligible countIf shape above the threshold uses helpers"
        );
        assert_eq!(BUDGET.helpers_in_use(), 0);

        let sequential = force_global_aggregate_workers(&mut database, 1, different_argument_sql);
        database.global_aggregate_parallelism =
            GlobalAggregateParallelism::budgeted(worker_cap, &BUDGET);
        let held = BUDGET
            .acquire_for_test(BUDGET.helper_limit())
            .expect("test exhausts aggregate helper admission");
        let exhausted = query(&mut database, different_argument_sql);
        assert_eq!(exhausted, sequential);
        assert_eq!(BUDGET.helpers_in_use(), BUDGET.helper_limit());
        drop(held);
        assert_eq!(BUDGET.helpers_in_use(), 0);
        ...
        let unsupported_sql =
            "SELECT active, countIf(active), countIf(included) FROM events GROUP BY active";
        let sequential = force_global_aggregate_workers(&mut database, 1, unsupported_sql);
        database.global_aggregate_parallelism =
            GlobalAggregateParallelism::budgeted(worker_cap, &BUDGET);
        BUDGET.reset_peak();
        assert_eq!(query(&mut database, unsupported_sql), sequential);
        assert_eq!(
            BUDGET.peak_helpers_in_use(),
            0,
            "non-sole countIf projections retain the complete sequential fallback"
        );
    }

    #[test]
    fn grouped_bool_count_if_forced_workers_preserve_resource_boundaries() {
        let row_count = GLOBAL_AGGREGATE_PARALLEL_ROW_THRESHOLD + 1;
        let mut database = count_if_database(row_count);
        let aggregate_state_bytes = 2_usize
            .saturating_mul(std::mem::size_of::<AggregateState>())
        ...
        let sql = "SELECT active, countIf(active) FROM events GROUP BY active";
    ...
    fn global_count_min_float64_parallel_boundary_and_admission_exhaustion_fall_back_sequentially()
    {
        ...
        assert!(
            OBSERVED_BUDGET.peak_helpers_in_use() > 0,
            "sole Bool-grouped MIN(Float64) above the threshold uses shared helpers"
        );
    ...
    fn global_count_max_float64_parallel_boundary_and_admission_use_sequential_fallback() {
        ...
        assert!(
            OBSERVED_BUDGET.peak_helpers_in_use() > 0,
            "sole Bool-grouped MAX(Float64) above the threshold uses the shared helper budget"
        );
    }
