...
use crate::batch::scalar_cast::{
    ...
    ordering_string_to_bool, ordering_string_to_float64, validate_string_to_bool,
    validate_string_to_float64, validate_string_to_int64,
};
use crate::batch::sql::{
        ...
        match items[order.output] {
            ResolvedItem::CastStringToInt64 { source } => {
                for row in rows.iter().copied() {
                    validate_string_to_int64(string_at(table, source, row))?;
                }
            }
            ResolvedItem::CastStringToFloat64 { source } => {
                for row in rows.iter().copied() {
                    validate_string_to_float64(string_at(table, source, row))?;
                }
            }
            ResolvedItem::CastStringToBool { source } => {
                for row in rows.iter().copied() {
                    validate_string_to_bool(string_at(table, source, row))?;
                }
            }
            _ => {}
        }
    ...
    sort_and_limit(rows, limit, |left, right| {
        for order in ordering {
            let comparison = match items[order.output] {
                ...
                ResolvedItem::CastStringToBool { source } => {
                    let left = ordering_string_to_bool(string_at(table, source, left));
                    let right = ordering_string_to_bool(string_at(table, source, right));
                    left.cmp(&right)
                }
                ResolvedItem::CastInt64ToString { source } => {
