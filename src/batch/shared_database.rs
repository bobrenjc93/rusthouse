...
/// [`Self::ingest_tsv_with_names`], and
/// [`Self::ingest_json_compact_each_row`] retain a write lock through their
/// complete bounded, atomic import operations.
///
/// # Examples
///
/// ```
...
impl SharedDatabase {
    ...
    /// Atomically ingests bounded, one-column `JSONCompactEachRow` bytes.
    ///
    /// One write lock is retained through target lookup, UTF-8 and complete
    /// input validation, configured and table-capacity checks, WAL logging,
    /// and the prepared-row append. Each line must contain one JSON array with
    /// one signed integer, or `null` for `Nullable(Int64)`.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// [`SharedDatabaseError::DatabaseBusy`] without inspecting either one.
    /// The acquired guard is retained through complete validation and commit.
