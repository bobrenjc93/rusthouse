...
pub enum Statement {
    ...
    /// Conditional exact `Nullable(Int64)` addition; an existing column is a
    /// case-insensitive no-op regardless of its physical representation.
    ...
    fn parse_alter(&mut self) -> Result<Statement> {
        ...
        if self.eat_keyword("ADD") {
            ...
            if if_not_exists {
                return Err(Error::Sql {
                    position,
                    message: "IF NOT EXISTS is supported only for Nullable(Int64) ADD COLUMN"
                        .to_owned(),
                });
            }
            if !self.at(&TokenKind::Semicolon) && !self.at(&TokenKind::End) {
