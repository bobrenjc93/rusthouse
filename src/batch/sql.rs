...
/// One `column IS NULL` or `column IS NOT NULL` atom in a mixed `DELETE` predicate.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DeleteNullnessPredicate {
    pub column: String,
    pub is_null: bool,
}
...
impl<'a> Parser<'a> {
    ...
    fn parse_select_statement(&mut self) -> Result<Statement> {
        ...
        if self.at_system_columns_select_start() {
            return self.parse_system_columns_select();
        }
    ...
    fn parse_system_columns_select(&mut self) -> Result<Statement> {
        const SHAPE: &str = "system.columns metadata supports exactly SELECT database, table, name, type, position FROM system.columns with no trailing clauses";

        for (index, column) in ["DATABASE", "TABLE", "NAME", "TYPE", "POSITION"]
            .into_iter()
            .enumerate()
        {
            self.reserve_ast_list_item()?;
            self.expect_keyword(column)?;
            if index != 4 {
                self.expect(&TokenKind::Comma, "',' between system.columns columns")?;
            }
        }
        self.expect_keyword("FROM")?;
        self.expect_keyword("SYSTEM")?;
        self.expect(&TokenKind::Dot, "'.' in system.columns")?;
        self.expect_keyword("COLUMNS")?;
        if !self.at(&TokenKind::Semicolon) && !self.at(&TokenKind::End) {
            return self.error(SHAPE);
        }

        Ok(Statement::SystemColumns)
    }

    fn parse_system_metrics_select(&mut self) -> Result<Statement> {
        const SHAPE: &str = "system.metrics supports exactly SELECT metric, value FROM system.metrics with no trailing clauses";

        for (index, column) in ["METRIC", "VALUE"].into_iter().enumerate() {
            self.reserve_ast_list_item()?;
            self.expect_keyword(column)?;
            if index == 0 {
                self.expect(&TokenKind::Comma, "',' between system.metrics columns")?;
            }
        }
        self.expect_keyword("FROM")?;
        self.expect_keyword("SYSTEM")?;
        self.expect(&TokenKind::Dot, "'.' in system.metrics")?;
    ...
    fn at_system_metrics_select_start(&self) -> bool {
        ...
        if !is_keyword(&self.current, "METRIC") {
            return false;
        }
        ...
        next_is_token(&mut lexer, TokenKind::Comma)
            ...
            && next_is_keyword(&mut lexer, "SYSTEM")
            && next_is_token(&mut lexer, TokenKind::Dot)
            && next_is_keyword(&mut lexer, "METRICS")
    }

    fn at_system_columns_select_start(&self) -> bool {
        fn is_keyword(token: &Token, expected: &str) -> bool {
            matches!(&token.kind, TokenKind::Identifier(value) if value.eq_ignore_ascii_case(expected))
        }
        ...
        let mut lexer = self.lexer;
        next_is_token(&mut lexer, TokenKind::Comma)
            && next_is_keyword(&mut lexer, "TABLE")
            && next_is_token(&mut lexer, TokenKind::Comma)
            && next_is_keyword(&mut lexer, "NAME")
            && next_is_token(&mut lexer, TokenKind::Comma)
            && next_is_keyword(&mut lexer, "TYPE")
            && next_is_token(&mut lexer, TokenKind::Comma)
            && next_is_keyword(&mut lexer, "POSITION")
            && next_is_keyword(&mut lexer, "FROM")
            && next_is_keyword(&mut lexer, "SYSTEM")
            && next_is_token(&mut lexer, TokenKind::Dot)
            && next_is_keyword(&mut lexer, "COLUMNS")
    }
    ...
    fn parse_delete_where(&mut self, table: String) -> Result<Statement> {
        ...
        if self.eat_keyword("IS") {
            let nullness = self.parse_delete_nullness_after_is(first_column)?;
            if self.eat_keyword("AND") {
                let comparison = self.parse_delete_comparison()?;
                self.record_predicate_node()?;
                if !self.at(&TokenKind::Semicolon) && !self.at(&TokenKind::End) {
                    return self.error(
                        "DELETE supports at most one nullness atom and one comparison joined by exactly one AND",
                    );
                }
            ...
            if !self.at(&TokenKind::Semicolon) && !self.at(&TokenKind::End) {
                return self.error(
                    "DELETE supports one nullness atom or one nullness atom and one comparison joined by exactly one AND",
                );
            }
            return Ok(Statement::DeleteNullness {
                table,
                column: nullness.column,
                is_null: nullness.is_null,
            });
        }
        ...
        if self.eat_keyword("AND") {
            ...
            if self.eat_keyword("IS") {
                let nullness = self.parse_delete_nullness_after_is(second_column)?;
                self.record_predicate_node()?;
                if !self.at(&TokenKind::Semicolon) && !self.at(&TokenKind::End) {
                    return self.error(
                        "DELETE supports at most one comparison and one nullness atom joined by exactly one AND",
                    );
                }
                return Ok(Statement::DeleteNullnessConjunction {
                    table,
    ...
    fn parse_delete_nullness_after_is(
        &mut self,
        column: String,
    ) -> Result<DeleteNullnessPredicate> {
        let is_null = !self.eat_keyword("NOT");
        self.expect_keyword("NULL")?;
        self.record_predicate_node()?;
        Ok(DeleteNullnessPredicate { column, is_null })
    }
