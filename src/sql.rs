...
impl Parser {
    ...
    fn parse_operand(&mut self, allow_aggregates: bool) -> Result<Operand> {
        match self.peek() {
            ...
            _ => self.error("expected column, aggregate, or literal"),
        }
    }
...
mod tests {
    ...
    fn parses_complete_select_shape() {
        let statements = parse(
            "SELECT region, SUM(amount) AS total FROM sales \
             ...
             GROUP BY region HAVING total >= 10 AND COUNT(*) > 1 \
    ...
    #[test]
    fn applies_predicate_complexity_limits_to_having() {
        let nested = format!(
            "SELECT id FROM things GROUP BY id HAVING {}id = 1{}",
            "(".repeat(MAX_PREDICATE_DEPTH + 1),
            ")".repeat(MAX_PREDICATE_DEPTH + 1)
        );
        let error = parse(&nested).expect_err("HAVING nesting limit should reject query");
        assert!(matches!(
            error,
            Error::Sql { message, .. }
        ...
        let predicate = vec!["COUNT(*) > 0"; MAX_PREDICATE_NODES + 1].join(" OR ");
        let flat = format!("SELECT id FROM things GROUP BY id HAVING {predicate}");
        let error = parse(&flat).expect_err("HAVING node limit should reject query");
        assert!(matches!(
            error,
            Error::Sql { message, .. }
                if message.contains("predicate is too complex; maximum 256 expression nodes")
