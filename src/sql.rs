...
impl Parser {
    ...
    fn parse_select(&mut self) -> Result<Select> {
        ...
        let table = self.expect_identifier("table name")?;
        let table_alias = self.parse_table_alias(&["INNER", "WHERE", "GROUP", "ORDER", "LIMIT"])?;
        ...
        if self.eat_keyword("INNER") {
            ...
            loop {
                let left = self.parse_column_name("column on the left side of JOIN equality")?;
                self.expect(&TokenKind::Equal, "'=' in INNER JOIN condition")?;
                let right = self.parse_column_name("column on the right side of JOIN equality")?;
