...
impl Parser {
    ...
    fn parse_copy(&mut self) -> Result<Statement> {
        ...
        let path = if let TokenKind::String(path) = self.peek().clone() {
            ...
            return self.error("expected CSV path as a string literal");
        };
        self.expect_keyword("FORMAT")?;
        self.expect_keyword("CSV")?;
        let header = if self.eat_keyword("WITH") {
...
mod tests {
    ...
    #[test]
    fn parses_copy_csv_with_optional_header() {
        let statements =
            parse("COPY Events FROM '/tmp/data.csv' FORMAT CSV WITH HEADER").expect("valid COPY");
