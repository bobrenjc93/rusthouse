    ...
    fn parse_operand(&mut self, allow_aggregates: bool) -> Result<Operand> {
        match self.peek() {
            ...
            TokenKind::Identifier(_) => {
                ...
                if !allow_aggregates {
                    return self.error("aggregate calls are not allowed in WHERE");
                }
                let (function, argument) = self.parse_aggregate_call(&name, position)?;
...
mod tests {
    ...
    fn parses_complete_select_shape() {
        let statements = parse(
            "SELECT region, SUM(amount) AS total FROM sales \
             ...
             GROUP BY region HAVING total > 5 OR COUNT(*) >= 2 \
    ...
    #[test]
    fn rejects_excessive_having_nesting_and_terms() {
        let depth = MAX_PREDICATE_DEPTH + 1;
        let nested = format!(
            "SELECT COUNT(*) FROM things HAVING {}COUNT(*) > 0{}",
            "(".repeat(depth),
            ")".repeat(depth)
        );
        let error = parse(&nested).expect_err("HAVING nesting is bounded");
        assert!(matches!(
            error,
            Error::Sql { message, .. } if message.contains("predicate nesting exceeds limit of 64")
        ));

        let terms = vec!["COUNT(*) > 0"; MAX_PREDICATE_NODES + 1].join(" OR ");
        let error = parse(&format!("SELECT COUNT(*) FROM things HAVING {terms}"))
            .expect_err("HAVING node count is bounded");
        assert!(matches!(
            error,
            Error::Sql { message, .. }
                if message.contains("predicate is too complex; maximum 256 expression nodes")
