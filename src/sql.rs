...
/// table and may contain a flat `WHERE` chain of column-to-literal comparisons
/// joined by `AND`. Literals may be `Int64`, `Float64`, `Bool`, or `String`.
/// That may be followed by one `ORDER BY column [ASC|DESC]` clause and a
/// nonnegative integer `LIMIT`. Other aliases, expressions, predicate forms,
/// and result modifiers are outside this intentionally narrow syntax boundary.
