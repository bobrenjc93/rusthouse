...
#[test]
fn rejects_missing_columns_grouped_queries_and_malformed_shapes() {
    let mut database = Database::new();
    database
        .execute(
            "CREATE TABLE readings (value Nullable(Int64)); \
    ...
    let grouped_error = Err(Error::InvalidQuery(
        "isNotNull projections are only supported in ungrouped SELECT queries".to_owned(),
    ));
    for sql in [
        "SELECT isNotNull(value), COUNT(*) FROM readings",
        "SELECT isNotNull(value) FROM readings GROUP BY value",
        "SELECT isNotNull(value), COUNT(*) FROM readings GROUP BY value",
    ] {
        assert_eq!(database.execute(sql), grouped_error, "{sql}");
    }
