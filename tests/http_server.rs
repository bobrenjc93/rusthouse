...
use rusthouse::{
    Database, QueryCancellation, QueryError, QueryFuture, QueryRequest, QueryResult, QueryService,
...
async fn start(service: Arc<dyn QueryService>, config: ServerConfig) -> (ServerHandle, String) {
...
async fn executes_sql_against_the_database_engine() {
    ...
    let create = client
        ...
        .body(
            "CREATE TABLE events (id Int64, label String, active Bool, score Float64, note String NULL)",
        )
        .send()
        .await
        .unwrap();
    ...
    assert_eq!(
        create.json::<Value>().await.unwrap(),
        json!([{"result": "CREATE TABLE"}])
    );

    let insert = client
        .post(format!("{url}/query"))
        .json(&json!({
            "query": "INSERT INTO events VALUES (1, 'alpha', true, 1.5, NULL), (2, 'beta', false, 2.25, 'done')"
        }))
        .send()
        .await
        .unwrap();
    ...
    assert_eq!(
        insert.json::<Value>().await.unwrap(),
    ...
    let rows = client
        .post(format!("{url}/query"))
        .header("content-type", "application/sql")
        .body("SELECT * FROM events")
        .send()
        .await
        .unwrap();
    assert_eq!(rows.status(), StatusCode::OK);
    assert_eq!(
        rows.json::<Value>().await.unwrap(),
        json!([
            {"id": 1, "label": "alpha", "active": true, "score": 1.5, "note": null},
            {"id": 2, "label": "beta", "active": false, "score": 2.25, "note": "done"}
        ])
    );
