...
fn parameterized_max_aggregate_state_cells_enforces_global_and_grouped_boundaries() {
    ...
    let cases = [
        (
            "SELECT+COUNT%28%2A%29%2C+SUM%28value%29+FROM+samples%3B",
            r#"{"columns":[{"name":"COUNT(*)","type":"Int64"},{"name":"SUM(value)","type":"Int64"}],"rows":[[2,30]]}"#,
        ),
        (
            "SELECT+g%2C+COUNT%28%2A%29+FROM+samples+GROUP+BY+g%3B",
            r#"{"columns":[{"name":"g","type":"Int64"},{"name":"COUNT(*)","type":"Int64"}],"rows":[[1,1],[2,1]]}"#,
        ),
    ];
...
#[test]
fn parameterized_max_aggregate_state_cells_zero_and_larger_values_cannot_relax_settings() {
    let configured_limits = QueryResultLimits {
    ...
    database
        ...
        .unwrap();
    let query = "SELECT+g%2C+COUNT%28%2A%29%2C+SUM%28value%29+FROM+samples+GROUP+BY+g%3B";
    let settings_before = exchange(&database, &request(b"SHOW SETTINGS"));
    ...
    for (method, requested_max) in [
        ...
        let request = format!(
            "{method} /?query={query}&max_aggregate_state_cells={requested_max} HTTP/1.1\r\nHost: localhost\r\n\r\n"
        );
        assert_response(
            &exchange(&database, request.as_bytes()),
            "HTTP/1.1 400 Bad Request",
            r#"{"error":"SELECT aggregate state cells requires at least 4, exceeding the limit of 3"}"#,
        );
    }
...
fn concurrent_parameterized_max_aggregate_state_cell_requests_are_isolated() {
    ...
    database
        ...
        .unwrap();
    let query = "SELECT+g%2C+COUNT%28%2A%29+FROM+samples+GROUP+BY+g%3B";
    let success_request = format!(
