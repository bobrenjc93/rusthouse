...
#[test]
fn parameterized_max_group_key_cells_enforces_all_exact_boundaries() {
    let database = SharedDatabase::default();
    database
        .execute(
            "CREATE TABLE samples (value Int64); \
    ...
    let cases = [
        (
            ...
            r#"{"columns":[{"name":"value","type":"Int64"},{"name":"n","type":"Int64"}],"rows":[[1,1],[2,1],[3,1]]}"#,
        ),
        (
        ...
        (
            "SELECT+value+FROM+samples+WHERE+value+%3C%3D+2+UNION+DISTINCT+SELECT+value+FROM+samples+WHERE+value+%3E%3D+2%3B",
            r#"{"columns":[{"name":"value","type":"Int64"}],"rows":[[1],[2],[3]]}"#,
        ),
    ];
    ...
    for method in ["GET", "POST"] {
        for (query, expected_body) in cases {
            let exact = format!(
                "{method} /?max_group_key_cells=3&query={query} HTTP/1.1\r\nHost: localhost\r\n\r\n"
            );
            assert_response(
                &exchange(&database, exact.as_bytes()),
                "HTTP/1.1 200 OK",
