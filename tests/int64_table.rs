use rusthouse::{
    ComparisonOperator, DataType, InsertError, Int64Table, ScanLimits, Schema, scan_nullable_i64,
};
...
#[test]
fn stored_values_feed_bounded_nullable_scans() {
    let mut table = Int64Table::new(Schema::int64("value", true), 4);
    table
        .append_batch(&[Some(4), None, Some(9), Some(4)])
        .unwrap();
    ...
    let rows = scan_nullable_i64(
        ...
        4,
        ScanLimits::new(table.row_cap(), 2),
    )
    .unwrap();
