...
fn caller_token_and_passthrough_wrapper_cannot_forge_attestation() {
    ...
    let authorization = repository
        ...
        .join(format!(
            ".rusthouse-malicious-wrapper-authorization-{}",
            std::process::id()
        ));
    fs::write(
        &authorization,
    ...
    .expect("malicious wrapper authorization");
    fs::set_permissions(&authorization, fs::Permissions::from_mode(0o600))
    ...
    fs::remove_file(authorization).expect("cleanup malicious authorization");
    assert!(!output.status.success());
    assert!(
        String::from_utf8_lossy(&output.stderr)
            .contains("attested workspace wrapper does not match builder authorization")
    );
}
...
fn attested_builder_uses_cargo_artifacts_from_nested_cwd_and_target_triple() {
    ...
    let rusthouse = release_dir.join(format!("rusthouse{}", env::consts::EXE_SUFFIX));
    let mut permissions = fs::metadata(&rusthouse)
        .expect("cached RustHouse metadata")
        .permissions();
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt as _;
    ...
    fs::set_permissions(&rusthouse, permissions).expect("make cached RustHouse replaceable");
...
impl TemporaryRepository {
    ...
    fn cargo_output_inner(
        ...
        command
            ...
            .env_remove("RUSTHOUSE_ATTESTED_BUILD")
            .env_remove("RUSTHOUSE_ATTESTED_BUILD_SESSION")
            .env_remove("RUSTHOUSE_ATTESTED_BUILD_AUTHORIZATION")
            .env_remove("RUSTHOUSE_ATTESTED_BUILD_TOKEN")
            .env_remove("RUSTHOUSE_ATTESTED_BINARY_SHA256")
            .current_dir(&self.path);
        ...
        if attested {
            ...
            let authorization = self
                ...
                .parent()
                .expect("repository parent")
                .join(format!(".rusthouse-attested-authorization-{session}"));
