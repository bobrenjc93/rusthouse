...
use rusthouse::batch::sql::{SUPPORTED_FUNCTION_NAMES, Statement, parse};
...
fn database_returns_the_exact_complete_function_inventory_in_stable_order() {
    ...
    assert_eq!(query(&mut database, "show functions;"), expected_result());
}

#[test]
fn show_functions_accepts_exact_and_rejects_exceeded_result_limits() {
...
#[test]
fn shared_database_admits_show_functions_on_both_read_paths_and_boundaries() {
...
#[test]
fn cli_emits_show_functions_in_every_output_format() {
    ...
    for (format, expected) in cases {
        let output = run_cli(format, b"SHOW FUNCTIONS;");
...
fn authenticated_read_only_http_paths_emit_every_wire_format() {
    ...
    for (format, expected) in cases {
        let response =
            authenticated_http_exchange(&database, "Authorization: Bearer read-token\r\n", format);
        assert!(
    ...
    let key_response =
        authenticated_http_exchange(&database, "X-ClickHouse-Key: read-key\r\n", None);
