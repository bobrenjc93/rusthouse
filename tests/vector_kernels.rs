...
#[test]
fn batch_type_errors_remain_distinct_from_database_type_errors() {
    let batch = RecordBatch::try_new(
        Schema::new(vec![Field::new("value", DataType::Int64, false)]),
        vec![Column::Int64(
            Int64Array::from_options(1, [Some(1)]).unwrap(),
        )],
        BatchConfig::unlimited(1),
    )
    .unwrap();

    assert_eq!(
        compare_string(&batch, 0, ComparisonOp::Eq, "1"),
        Err(Error::BatchTypeMismatch {
            column: 0,
            expected: "String",
            actual: "Int64",
        })
